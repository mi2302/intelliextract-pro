...
async function run() {
    ...
    try {
        ...
        const objectsSql = `
            SELECT DATA_IDENTIFIER, TABLE_NAME, MAPPED_COLUMN_NAME, COLUMN_NAME, METADATA_COLUMN_HEADER
            FROM XX_INTELLI_RECON_TAB_COLUMN_MAPPING 
            WHERE DATA_IDENTIFIER = 'GlInterface'
            AND ROWNUM <= 20
        `;

        const objectsResult = await conn.execute(objectsSql, {}, { outFormat: oracledb.OUT_FORMAT_OBJECT });
        console.log("Sample Data Headers:", objectsResult.rows);
